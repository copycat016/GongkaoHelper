...
### 方式 B：公网 + Cloudflare Tunnel（绑域名、自带 HTTPS、无需开放端口）

程序与上面完全相同，只是把访问层从 Tailscale 换成 Cloudflare Tunnel。
前提：域名已托管在 Cloudflare。

```bash
# 1. 装 cloudflared
curl -fsSL https://pkg.cloudflare.com/install.sh | sudo bash   # 或从官网下载对应包
sudo apt install -y cloudflared

# 2. 登录并创建隧道（浏览器授权你的 Cloudflare 账号）
cloudflared tunnel login
cloudflared tunnel create gkweb            # 记下生成的 Tunnel ID / 凭证文件路径

# 3. 把域名指向本机的 21080
#    在 ~/.cloudflared/config.yml 写：
#      tunnel: <Tunnel ID>
#      credentials-file: /root/.cloudflared/<Tunnel ID>.json
#      ingress:
#        - hostname: gk.你的域名.com
#          service: http://localhost:21080
#        - service: http_status:404
cloudflared tunnel route dns gkweb gk.你的域名.com
...
之后浏览器访问 `https://gk.你的域名.com` 即可，TLS 由 Cloudflare 自动签发，**VPS 不用开放任何公网入站端口**。

> ⚠️ 走公网后，`backend/.env` 里的 `GIN_MODE=release` / 强 `JWT_SECRET` / 强 `AUTH_BOOTSTRAP_PASSWORD` 从「建议」变成「必须」。
